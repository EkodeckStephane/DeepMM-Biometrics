...
| **G8 — Reproducibility** | Code, configs, split manifests, trials, seeds, raw scores, hashes, environment, regeneration scripts; restricted datasets referenced rather than redistributed. | **ADVANCED INFRASTRUCTURE / FINAL EVIDENCE OPEN** | CI, metrics, classical baselines, D1/D2/D3S/D3F neural modules, matched search-budget contracts, fit/selection/calibration/final-test firewall, deterministic development trainer, checkpoint hashing, dataset-manifest audit, statistics, split/trial/score hashing and immutable run provenance are versioned. Final real-data manifests, frozen search spaces/configs, encoder checkpoints/environments, raw scores and regeneration scripts remain. |
...
7. Raw neural logits are reported/interpreted as calibrated likelihood ratios without an explicit held-out calibration procedure.
8. Robustness uses corruptions/severities selected after seeing which model benefits.
9. Cost comparisons use different hardware, batch size, precision, or measurement procedures without correction/disclosure.
10. Technical training repetitions, finger instances, or raw verification-pair counts are treated as independent human subjects.
11. For NUPT-FPV or another multi-finger dataset, different fingers from the same volunteer cross outer train/development/calibration/test partitions.
12. Fit, model-selection, score-calibration and final-test evidence are not separated according to the frozen firewall/split protocol.
13. A one-way cluster bootstrap is used on dense symmetric all-vs-all impostor pairs without validating the dependence structure.
14. A model-family ranking is declared universal from a single dataset without bounded claims or generalization evidence.
15. A Transformer/attention/missing-modality mechanism is presented as novelty even though that mechanism is already established prior art.
16. Fixed-model classical score/feature fusion is presented as novel despite the OU-MB 2026 baseline precedent.
...
- Are fitting, early stopping/model selection, score calibration and final testing separated without information leakage?
...
**Gate 4 is closed at PASS-POSITIONING. NUPT-FPV is now the P1 access candidate, but Gate 5 remains open until the delivered archive and full trial/statistical contract are audited and frozen. The neural development stack now has an explicit final-test firewall and deterministic checkpointed training path. GO for access acquisition, archive audit, encoder feasibility work and non-final development pilots. NO-GO for any confirmatory final-test campaign or article performance conclusion.**
